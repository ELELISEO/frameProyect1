...
router.post("/almacen", async (req, res) => {
  try {
    ...
    const query = await db.query(
      `INSERT INTO almacen (contacto, producto, proveedor, precio, cantidad) VALUES (?, ?, ?, ?, ?)`,
      [contacto, producto, proveedor, precio, cantidad]
    );
    const query2 = await db.query(
      `INSERT INTO inventario (producto, stock, venta_cantidad, almacen_almacen, total_carrito) VALUES (?, 0, 0, ?, ?)`,
...
router.delete("/almacen/deleteId/:id", async (req, res) => {
  try {
    ...
    if (result.rowCount > 0) {
      return res
        .status(200)
        .json({ status: 200, mensaje: "Proveedor eliminado correctamente" });
    } else {
      return res
        .status(404)
        .json({ status: 404, mensaje: "Proveedor no encontrado" });
    }
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .json({ status: 500, mensaje: "Error interno del servidor" });
  }
});
...
router.get("/producto/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const query = await db.query(
      `SELECT cantidad, precio FROM almacen WHERE id = '${id}';`
    );
    if (query.length > 0) {
...
router.put("/almacen/cantidad/:id", async (req, res) => {
  ...
  if (isNaN(cantidad) || cantidad < 0) {
    return res.status(400).json({
      error: "Cantidad debe ser un número válido y mayor o igual a 0",
    });
  }

  try {
    // Seleccionamos la cantidad actualizada para devolverla como respuesta
    const selectQuery = "SELECT producto, cantidad FROM almacen WHERE id = ?";
    const rows = await db.query(selectQuery, [id]);    
    ...
    const result = await db.execute(updateQuery, [rows[0].cantidad + cantidad, id]);

    // Actualizamos la cantidad en la tabla inventario
    const updateQuery2 = `
                  UPDATE inventario 
                  SET almacen_almacen = ? 
                  WHERE producto = ?;
    ...
    const result2 = await db.execute(updateQuery2, [
    ...
    res
      .status(500)
      .json({ error: "Hubo un error al actualizar la cantidad del producto" });
  }
});
